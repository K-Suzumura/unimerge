...
同一写真と判定する条件（いずれか）:
  1. ImageUniqueID が一致
  2. DateTimeOriginal + Make + Model が完全一致
  3. DateTimeOriginal + GPS座標(誤差10m以内) が一致
  4. OriginalRawFileName が一致 かつ DateTimeOriginal が一致
```
...
| 項目 | 説明 |
|------|------|
| 記録方式 | SQLiteデータベースに全操作履歴を記録 |
| ロールバック | unimerge.exe /undo [JobID] で逆操作を生成し、元の状態に復元 |
| 保持期間 | 設定可能（デフォルト: 30日間） |
